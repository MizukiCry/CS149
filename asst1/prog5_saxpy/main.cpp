...
void saxpyCustom(int N, float scale, float X[], float Y[], float result[]) {
  ispc::saxpy_ispc(N, scale, X, Y, result);
}
...
int main() {
  ...
  //
  double minCustom = 1e30;
  for (int i = 0; i < 3; ++i) {
    double startTime = CycleTimer::currentSeconds();
    saxpyCustom(N, scale, arrayX, arrayY, resultTasks);
    double endTime = CycleTimer::currentSeconds();
    minCustom = std::min(minCustom, endTime - startTime);
  }

  verifyResult(N, resultTasks, resultSerial);

  printf("[saxpy custom]:\t\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
         minCustom * 1000, toBW(TOTAL_BYTES, minCustom),
         toGFLOPS(TOTAL_FLOPS, minCustom));
